...
\fancyformula{Verhältnis von $I_{1W}$ zum elektrischen Polradwinkel}{
	\[
		I_{1W}=I_1~\cos(\phi)=\dfrac{U_p}{\omega~L_1}~\sin(-\vartheta)=\dfrac{U_{1N}}{\omega~L_1}~\dfrac{I_e}{I_{e0}}~\sin(-\vartheta)
	\]
}

\fancyformula{Drehmoment in Abhängigkeit von Polradwinkel}{
	\[
		M_i=M_{iK}~\sin(-\vartheta)=3~\dfrac{p}{\omega^2~L_1}~U_1~U_{1N}~\dfrac{I_e}{I_{e0}}~\sin(-\vartheta)
	\]
	\begin{center}
	Wobei: $M_{iK}=3~\dfrac{p}{\omega^2~L_1}~U_1~U_{1N}~\dfrac{I_e}{I_{e0}}$
	\end{center}
}
